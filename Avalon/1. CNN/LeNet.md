...
for the global loss function $E^p (Z^p, W)$ is differentiable, the rules are we must built the system is feed-forward network of differentiable modules overall. The function that implemented by each modul must be continuous and differentiable almost everywhere with respect to the internal parameters pf the module (what LeCun trying to say is, $W, b$. this both must in a form of continuous). so basically $ W, b$ can be differentiable with respect of the module's input

$$\begin{pmatrix} \frac{\partial E}{\partial W} , \frac{\partial E}{\partial b} \end{pmatrix}$$

can compute when backprop. So yeah with this condition back-propagation can be used for compute the gradients of the loss function with respect to all the parameters in the systemn. now lets say we have a modules with a function $X_n =F_n(Wn, X_{n-1})$, $X_n$ as vector representing the output, $W_n$ as vector for parameter that can be tune (not all of the parameter, just a subset of $W$), and lastly $X_{n-1}$ as vector for the input for the module (or called it previous output). The input ($X_0$) to the first module is the input pattern $Z^p$. if the partial derivative of the loss function ($E^p$) with respect to $X_n$ is known, then the partial derivative of loss function with respect to $W_n$ and $X_{n-1}$ can be computed 
...
$\frac{\partial F}{\partial W}(W_n, X_{n-1})$ is the Jacobian of $F$ with respect to $W$ doing evaluation at the point ($W_n , X_{n-1}$), and $\frac{\partial F}{\partial X}(W_n, X_{n-1})$ is the Jacobian of $F$ with respect of $X$. The Jacobian of a vector function is really just a matrix containing the partial derivatives of all the outputs with respect to all the inputs. The first equation computes some terms of the gradient of $E^p (W)$, while the second one generates a backward reccurence, as in the well-known back-propagation procedure for neural networks. Later we can average the gradients from training process to get the full gradient. LeCun note that "It is interesting to note that in many instances there is no need to explicitly compute the Jacobian matrix The above formula uses the product of the Jacobian with a vector of partial derivatives, and it is often easier to compute this product directly without computing the Jacobian beforehand". the analogy state that in ordinary multi-layer neural-networks, all but the last module are called hidden layers because their outputs are not observable from the outside more complex situations than the simple cascade of modules described above, the partial derivative notation becomes somewhat ambiguous and awkward A completely rigorous derivation in more general cases can be done using Lagrange functions 
